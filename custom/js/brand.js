	...
	if(brandId) {
		...
		$.ajax({
			...
			success:function(response) {
				...
				$('#editBrandName').val(response.companies_name);
				// brand id 
				$(".editBrandFooter").after('<input type="hidden" name="brandId" id="brandId" value="'+response.companies_id+'" />');
...
function removeBrands(brandId = null) {
	if(brandId) {
		...
		$.ajax({
			...
			dataType: 'json',
			success:function(response) {
				$('.removeBrandFooter').after('<input type="hidden" name="removeBrandId" id="removeBrandId" value="'+response.companies_id+'" /> ');
