	   	...
	   	for (var x = 0; x < quantity.length; x++) {       						
			...
			if(orderDate && clientName && clientContact && paid && discount && paymentType && paymentStatus) {
				if(validateProduct == true && validateQuantity == true) {
	            	...
	            	'<a href="recipe.php?r=add" class="btn btn-default" style="margin-left:10px;"> <i class="glyphicon glyphicon-plus-sign"></i> Yeni Sipariş </a>'+
...
function printOrder(orderId = null) {
	if(orderId) {		
		...
		$.ajax({
			url: 'php_action/printRecipe.php',
			type: 'post',
...
function paymentOrder(orderId = null) {
	if(orderId) {
		...
		$.ajax({
			url: 'php_action/fetchRecipeData.php',
			type: 'post',
			data: {orderId: orderId},
			dataType: 'json',
			success:function(response) {				
